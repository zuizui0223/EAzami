...
class NewickParser:
    def __init__(self, text: str):
        self.s = text.strip(); self.i = 0
        self.tips: list[str] = []; self.edge_lengths: list[float] = []
        self.missing_edge_lengths = 0; self.clades: list[frozenset[str]] = []
    ...
    def label(self) -> str:
        ...
        if self.i < len(self.s) and self.s[self.i] in "'\"":
            ...
            while self.i < len(self.s):
                ...
                if c==q:
                    if self.i < len(self.s) and self.s[self.i]==q: out.append(q); self.i+=1; continue
                    break
                out.append(c)
    ...
    def branch_length(self, *, required: bool) -> None:
        ...
        if self.i >= len(self.s) or self.s[self.i] != ":":
            if required: self.missing_edge_lengths += 1
            return
        self.i+=1; self.skip_ws(); start=self.i
        while self.i < len(self.s) and self.s[self.i] not in ",();": self.i+=1
        raw=self.s[start:self.i].strip()
        try: value=float(raw)
        except ValueError as exc: raise ValueError(f"Invalid Newick branch length {raw!r}") from exc
        if not math.isfinite(value) or value < 0: raise ValueError(f"Branch length must be finite and >=0, observed {value}")
        self.edge_lengths.append(value)

    def subtree(self, *, is_root: bool=False) -> set[str]:
        self.skip_ws()
        if self.i >= len(self.s): raise ValueError("Unexpected end of Newick")
        if self.s[self.i] == "(":
            self.i+=1; descendants=set(self.subtree())
            while True:
                self.skip_ws()
                if self.i < len(self.s) and self.s[self.i] == ",": self.i+=1; descendants.update(self.subtree()); continue
                break
            self.skip_ws()
            if self.i >= len(self.s) or self.s[self.i] != ")": raise ValueError("Unbalanced Newick parentheses")
            self.i+=1; _=self.label(); self.branch_length(required=not is_root)
            self.clades.append(frozenset(descendants)); return descendants
        lab=self.label()
        if not lab: raise ValueError("Leaf tip has empty label")
        self.tips.append(lab); self.branch_length(required=True); return {lab}

    def parse(self) -> tuple[list[str], list[float], int]:
        self.subtree(is_root=True); self.skip_ws()
        if self.i < len(self.s) and self.s[self.i] == ";": self.i+=1
        self.skip_ws()
        if self.i != len(self.s): raise ValueError(f"Unexpected trailing Newick content at offset {self.i}")
        if len(self.tips) != len(set(self.tips)): raise ValueError("Tree contains duplicate tip labels")
...
def eligible_atlas_taxa(path: Path) -> tuple[set[str], dict[str, str]]:
    ...
    if len(taxa)!=len(eligible): raise ValueError("Eligible atlas rows must be unique taxon-level records")
    states={r["accepted_taxon"]:r["binary_colour_code"].upper() for r in eligible}
    if any(v not in {"C","W"} for v in states.values()): raise ValueError("Eligible atlas taxa must have fixed C/W states")
    return taxa,states


def validate_mapping(path: Path, eligible:set[str], tree_tips:set[str]) -> dict[str,str]:
    rows=read_csv(path); required={"tree_tip","accepted_taxon","mapping_status"}
    if not rows or not required.issubset(rows[0]): raise ValueError(f"Tip map requires columns {sorted(required)}")
    mapping={}; seen=set()
    for row in rows:
        tip,taxon,status=row["tree_tip"],row["accepted_taxon"],row["mapping_status"]
        if status not in ALLOWED_MAPPING: raise ValueError(f"Unaccepted mapping_status {status!r} for {tip}")
        if tip in seen or taxon in mapping: raise ValueError("Tip map must be one-to-one")
        seen.add(tip); mapping[taxon]=tip
    missing_taxa=sorted(eligible-set(mapping))
    if missing_taxa: raise ValueError(f"Eligible atlas taxa missing from tip map: {missing_taxa}")
    missing_tree=sorted({mapping[t] for t in eligible}-tree_tips)
    if missing_tree: raise ValueError(f"Mapped tips absent from tree: {missing_tree}")
    return mapping
...
def _string_list(payload:dict[str,object], key:str) -> list[str]:
    ...
    if not isinstance(value,list) or any(not isinstance(x,str) or not clean(x) for x in value):
        raise ValueError(f"{key} must be a list of non-empty strings")
    out=[clean(x) for x in value]
    if len(out)!=len(set(out)): raise ValueError(f"{key} contains duplicates")
    return out


def validate_provenance(path: Path, tree_sha:str) -> dict[str,object]:
    p=json.loads(path.read_text(encoding="utf-8"))
    if p.get("tree_route") not in ALLOWED_ROUTES: raise ValueError("tree_route must be published_exact, compatibility_reanalysis, or independent_reanalysis")
    if p.get("tree_sha256") != tree_sha: raise ValueError("Provenance tree_sha256 does not match tree bytes")
    for key in ("analysis_name","branch_length_interpretation","rooting_definition","support_metric_definition","source_or_pipeline_provenance"):
        if not clean(p.get(key)): raise ValueError(f"Tree provenance lacks {key}")
    ...
    outgroups=_string_list(p,"required_outgroup_tips")
    references=_string_list(p,"required_reference_tips") if "required_reference_tips" in p else list(outgroups)
    if not set(outgroups) <= set(references): raise ValueError("required_outgroup_tips must be a subset of required_reference_tips")
    p["required_outgroup_tips"]=outgroups; p["required_reference_tips"]=references
    return p


def validate(tree:Path, atlas:Path, tip_map:Path, provenance:Path) -> dict[str,object]:
    tree_sha=sha256(tree); parser=NewickParser(tree.read_text(encoding="utf-8")); tips,lengths,missing=parser.parse(); tree_tip_set=set(tips)
    if missing: raise ValueError(f"Tree has {missing} non-root edges without branch lengths")
    if not lengths or not any(x>0 for x in lengths): raise ValueError("Tree has no positive empirical branch lengths")
    eligible,states=eligible_atlas_taxa(atlas); mapping=validate_mapping(tip_map,eligible,tree_tip_set); prov=validate_provenance(provenance,tree_sha)

    focal_tips={mapping[t] for t in eligible}
    root_outgroups=set(prov["required_outgroup_tips"]); references=set(prov["required_reference_tips"])
    focal_monophyly_checked=bool(references)
    if references:
        missing_refs=sorted(references-tree_tip_set)
        if missing_refs: raise ValueError(f"Required outgroup/reference tips absent from tree: {missing_refs}")
        overlap=sorted(references & focal_tips)
...
def main()->int:
    ap=argparse.ArgumentParser();ap.add_argument("--tree",type=Path,required=True);ap.add_argument("--atlas",type=Path,required=True);ap.add_argument("--tip-map",type=Path,required=True);ap.add_argument("--provenance",type=Path,required=True);ap.add_argument("--output",type=Path);a=ap.parse_args()
    out=validate(a.tree,a.atlas,a.tip_map,a.provenance); text=json.dumps(out,indent=2,ensure_ascii=False)+"\n"
