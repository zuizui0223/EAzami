...
def read_optimized_trees(path: Path, n: int):
    ...
    for line in lines[:n]:
        # IQ-TREE prefixes lines with comments such as: [ tree 1 lh=... ]
...
def brownian_covariance(tree, tip_names: list[str]) -> np.ndarray:
    ...
    # numerical nugget only; it is many orders below observed branch lengths
    cov += np.eye(n) * 1e-10
    return cov
...
def fit_pgls(y: np.ndarray, state: np.ndarray, cov: np.ndarray) -> dict[str, float]:
    ...
    return {
        "beta_D_minus_U_sd": float(beta[1]),
        "se": float(se[1]),
        "t": t_value,
        "p": p_value,
        "dof": int(dof),
...
def main() -> None:
    ...
    occ = occ.loc[occ.get("environment_complete", True).astype(bool)].copy()
    ...
    summary = (
        result_df.groupby("axis")
        .agg(
            beta_min=("beta_D_minus_U_sd", "min"),
            beta_max=("beta_D_minus_U_sd", "max"),
            p_min=("p", "min"),
            p_max=("p", "max"),
        )
        .reset_index()
    ...
    payload = {
        "contract_version": "fdt4_orientation_niche_pgls_v1",
        "usable_taxa": usable,
        "n_taxa": len(usable),
        "n_U": int((state == 0).sum()),
        "n_D": int((state == 1).sum()),
        "topologies_propagated": args.nonrejected_count,
        "min_occurrence_n_per_taxon": args.min_n,
        "axes": AXES,
        "interpretation_boundary": (
            "Present-day taxon niche centroids tested with Brownian phylogenetic covariance. "
            "This is a topology-sensitive screening association, not ancestral niche reconstruction, "
            "historical range inference, parallel adaptation, or causal evidence."
        ),
    }
    (args.out_dir / "fdt4_orientation_niche_pgls_manifest_v1.json").write_text(
