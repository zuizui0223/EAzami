...
def japan_wide_tree_gate() -> dict:
    ...
    return {
        ...
        "decision": (
            "tree_ready_for_japan38_trait_history"
            if tree_present
            else "sample_composition_and_execution_graph_ready_but_machine_tree_missing; do_not_infer_Japan38_transition_counts"
        ),
...
def layer_readiness(tree_gate: dict) -> list[dict[str, str]]:
    return [
        {
            "analysis_id": "FDT1",
            "status": "seed_registry_ready_effect_harmonization_next",
            "usable_now": "seven existing evidence families registered, including exact gradients, direct lnRR, structured gates and module-specific targets",
            "next_gate": "derive harmonized trait->function rows only where estimands are commensurable; do not force heterogeneous metrics into one pool",
        },
        {
            "analysis_id": "FDT2",
            ...
            "next_gate": "georeference homologous FDT1 effect rows; current niche audit is not a substitute for study-level effect geography",
        },
        {
            "analysis_id": "FDT3",
            "status": "partial_ready_tree_gated",
            "usable_now": "orientation repeated-state result; balanced Japan38 stickiness states; frozen exploratory phyllary harmonization",
            "next_gate": (
                "run Japan38 stickiness transition preflight"
                if tree_gate["machine_readable_japan_wide_tree_present"]
                else "execute/freeze the Japan-wide machine-readable nuclear tree before stickiness transition counts; meanwhile continue FDT1"
            ),
        },
        {
            "analysis_id": "FDT4",
            "status": "partial_ready",
            "usable_now": "focal occurrence niche pipeline; published temporal anchors; empirical focal branch-length tree; trait seeds",
