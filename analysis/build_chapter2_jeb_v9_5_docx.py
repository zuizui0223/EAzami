...
Inputs are the active markdown manuscript and the rendered V9.5 PNG figures.
The builder intentionally omits submission-preparation notes and does not invent
author or affiliation information.
"""
...
def set_image_alt(inline_shape, alt: str) -> None:
    ...
    docPr.set("descr", alt)
    docPr.set("title", alt[:120])


def add_figure(doc: Document, fig_n: int, fig_dir: Path, legend: str, alt: str) -> None:
    stems = {
        1: "figure1_repeated_components.png",
        2: "figure2_depth_ordering.png",
        3: "figure3_shared_localization.png",
        4: "figure4_representation_dependent_ecology.png",
    }
...
def extract_figure_legends(text: str):
    if "# Figure legends" not in text or "# References" not in text:
        raise ValueError("expected Figure legends and References sections")
    main, rest = text.split("# Figure legends", 1)
    figs, tail = rest.split("# References", 1)
    legends = {}
    alts = {}
    ...
    for m in pattern.finditer(figs.strip()):
        ...
        desc = clean_inline(parts[0])
        alt = clean_inline(parts[1]) if len(parts) == 2 else title
        legends[n] = f"{title} {desc}".strip()
        alts[n] = alt
...
def tokenize_blocks(text: str):
    ...
    for line in lines:
        ...
        if s.strip() == "\\]":
            blocks.append(("math", " ".join(mathbuf).strip())); in_math = False; mathbuf = []; continue
        if in_math:
            mathbuf.append(s.strip()); continue
        if not s.strip():
            if buf:
                blocks.append(("p", " ".join(x.strip() for x in buf).strip())); buf = []
            continue
...
def build_main(fig_dir: Path, out: Path) -> None:
    ...
    # Drop repository-facing metadata at the top; keep title and scholarly manuscript only.
    main = re.sub(r"\n\*\*Target journal:.*?\n\*\*Running title:.*?\n", "\n", main, flags=re.S)
    # Remove submission-preparation notes from tail.
    refs_tail = refs_tail.split("## Submission-preparation notes", 1)[0].strip()
    text = main + "\n\n" + refs_tail

    doc = Document()
    set_doc_defaults(doc, anonymous=True)
    ...
    for kind, content in tokenize_blocks(text):
        ...
        if kind == "h1":
            if content == clean_inline(raw.splitlines()[0].lstrip("# ")):
                ...
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                p.add_run(clean_inline(content)).bold = True
            else:
                doc.add_paragraph(clean_inline(content), style="Heading 1")
        elif kind == "h2":
            doc.add_paragraph(clean_inline(content), style="Heading 2")
        elif kind == "bullet":
            p = doc.add_paragraph(style="List Bullet")
            p.add_run(clean_inline(content))
        elif kind == "math":
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            ...
            p.add_run(clean_inline(content))
            for n in range(1, 5):
                if n not in inserted and re.search(rf"\(Figure {n}\)", content):
                    add_figure(doc, n, fig_dir, legends[n], alts[n])
                    inserted.add(n)

    # If a figure call was lost during markdown parsing, fail rather than silently omit it.
    if inserted != {1, 2, 3, 4}:
        raise AssertionError(f"not all figures embedded: {sorted(inserted)}")
...
def build_title_page(out: Path) -> None:
    doc = Document()
    set_doc_defaults(doc, anonymous=False)
    p = doc.add_paragraph(style="Title")
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.add_run("Repeated reassembly of a complex reproductive phenotype across unequal evolutionary depths in a young thistle radiation").bold = True
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.add_run("Research Article - Journal of Evolutionary Biology")
    doc.add_paragraph()
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.add_run("Author names, affiliations, corresponding-author details and acknowledgements to be added at submission freeze.").italic = True
    doc.add_paragraph()
    p = doc.add_paragraph()
    p.add_run("Running title: ").bold = True
    p.add_run("Unequal-depth capitulum reassembly")
    p = doc.add_paragraph()
    p.add_run("Data/code archive: ").bold = True
    p.add_run("Immutable DOI to be inserted after manuscript-evidence synchronization.")
    out.parent.mkdir(parents=True, exist_ok=True)
    doc.save(out)
...
def main() -> None:
    ...
    print(a.output_dir / "EAzami_JEB_V9_5_anonymous_main.docx")
    print(a.output_dir / "EAzami_JEB_V9_5_title_page.docx")
