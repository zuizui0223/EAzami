...
def main():
    ...
    dominant_ids=sorted(set(by)-DOMINANT_RADIATION_EXCEPTIONS);dominant_tips=[tip for mid in dominant_ids for tip in by[mid]]
    dom=mrca_for_tips(tree,dominant_tips);dom_desc=descendants(dom);dom_extra=sorted({rev[x] for x in dom_desc-set(dominant_tips)})
    dominant_monophyletic=(dom_desc==set(dominant_tips))
    ta01_tips=[by[mid][0] for mid in TA01_PAIR];anchor=mrca_for_tips(tree,ta01_tips);anchor_desc=descendants(anchor);anchor_concepts=sorted({rev[x] for x in anchor_desc})
    ...
    result={
      "contract_version":"japan38_time_calibration_audit_v1","tree_branch_length_unit":"substitutions_per_site","paper_concepts":38,"biological_tips":39,
      "dominant_radiation_membership_hypothesis":{"source_claim":"36/38 paper concepts in one dominant Japanese radiation","predeclared_secondary_history_concepts":sorted(DOMINANT_RADIATION_EXCEPTIONS),"compatibility_tree_monophyletic":dominant_monophyletic,"extra_concepts_inside_mrca":dom_extra,"ev01_may_be_used_as_direct_crown_calibration":False,"reason":"EV01 is a reconstructed range/opportunity event, and the published 36/38 membership is not monophyletic on the independent compatibility tree."},
      "directly_mappable_interval_anchor":{"event_id":"TA01","endpoint_concepts":list(TA01_PAIR),"endpoint_tips":ta01_tips,"point_age_ma":float(events["TA01"]["point_age_ma"]),"young_bound_ma":float(events["TA01"]["young_bound_ma"]),"old_bound_ma":float(events["TA01"]["old_bound_ma"]),"mrca_biological_tips":len(anchor_desc),"mrca_paper_concepts":len(anchor_concepts),"paper_concepts_outside_mrca":outside_anchor,"eligible_for_sensitivity_calibration":True,"eligible_as_ecological_event_window":False},
      "other_anchors":{"TA02":"clade membership not frozen at exact Japan38 concept level; chronology context only","TA03":"clade membership not frozen at exact Japan38 concept level; chronology context only","TA04":"endpoint taxa absent from Japan38 tree" if not ta04_present else "endpoints present"},
      "readiness":{"relative_branch_length_history_ready":True,"single_anchor_chronogram_sensitivity_ready":True,"confirmatory_absolute_time_tree_ready":False,"fdt6_event_correspondence_ready":False,"fdt5_absolute_time_disparity_ready":False},
