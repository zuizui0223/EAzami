...
mapping mode.  The predeclared Moreyra conservative-241 list is the locus
universe; loci are retained for this cross-assay tree only when at least 80% of
the 20 frozen focal taxa have a recovered sequence.  Original Compositae1061
reference sequences are appended as explicit non-Cirsium anchors/outgroups.

This script does not infer a tree and does not alter colour states.
"""
...
def loci(path: Path)->list[str]:
    ...
    if len(values)!=241 or len(set(values))!=241: raise ValueError("Expected frozen 241 unique loci")
    return values
...
def normalize_focal_records(records:list[tuple[str,str]], tip_ids:set[str])->list[tuple[str,str]]:
    ...
    for header,seq in records:
        ...
        # retrieve_sequences headers normally begin with the sample name; allow
        # exact tip IDs or tip ID followed by separators while rejecting guesses.
...
def build(primary:Path,locus_list:Path,retrieved:Path,target:Path,outdir:Path,min_fraction:float=0.8)->dict[str,object]:
    ...
    for locus in wanted:
        ...
        row={"locus":locus,"focal_sequences":len(focal),"focal_fraction":len(focal)/20,"reference_sequences":len(anchors),"has_lett":any(x[0]=="OUTGROUP_lett" for x in anchors),"has_sunf":any(x[0]=="OUTGROUP_sunf" for x in anchors),"eligible":False,"reason":""}
        if len(focal)<min_n: row["reason"]="focal_occupancy_below_0.80"
        elif not row["has_lett"] or not row["has_sunf"]: row["reason"]="required_reference_outgroup_missing"
        else:
            row["eligible"]=True; row["reason"]="eligible"; eligible.append(locus); write_fasta(locus_dir/f"{locus}.fasta",focal+anchors)
        manifest.append(row)
    ...
    summary={"contract_version":"colour_rate_comp1061_tree_inputs_v1","frozen_locus_universe":241,"focal_taxa":20,"minimum_focal_occupancy_fraction":min_fraction,"minimum_focal_sequences":min_n,"eligible_loci":len(eligible),"required_outgroups":["OUTGROUP_lett","OUTGROUP_sunf"],"tree_input_ready":len(eligible)>=100,"claim_limit":"The >=100 eligible-locus threshold is a conservative engineering gate for launching the tree stage, not a statistical guarantee of topology or rate-model adequacy."}
    (outdir/"tree_input_summary.json").write_text(json.dumps(summary,indent=2)+"\n",encoding="utf-8")
    if not summary["tree_input_ready"]: raise ValueError(f"Only {len(eligible)} of 241 loci passed the cross-assay occupancy/outgroup gate")
    return summary
