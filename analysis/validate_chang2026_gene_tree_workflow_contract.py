...
layout, the single canonical restartable assembly runner, complete Snakemake DAG
and conda files. It does not download reads or execute external bioinformatics
tools.
"""
...
def repo_root_from_snakefile(snakefile: Path) -> Path:
    ...
    if resolved.name != "Snakefile" or resolved.parent.name != "chang2026_gene_trees":
        raise ValueError(f"Expected workflow/chang2026_gene_trees/Snakefile, observed {resolved}")
...
def validate_workflow_files(snakefile: Path) -> tuple[list[str], dict[str, str], dict[str, str]]:
    ...
    if rules != list(EXPECTED_RULES):
        raise ValueError(f"Unexpected Snakefile rules/order: observed={rules}, expected={list(EXPECTED_RULES)}")
...
def validate_hypothesis_contract(path: Path) -> tuple[list[dict[str, object]], dict[str, int]]:
    ...
    topologies = [row.get("topology_newick", "") for row in rows]
    ...
    expected = {"nearest_loss_only_topology": 7, "topology_supported_candidate_regain": 1}
    if dict(classes) != expected:
        raise ValueError(f"Unexpected hypothesis classes: observed={dict(classes)}, expected={expected}")
    ...
    if any(str(row.get("rooted_rf_distance_from_published")) != "4" for row in loss):
...
def build_contract(
    ...
    panel_rows = assembly_runner.validate_panel(panel, expected_samples=19)
    ...
    if dict(role_counts) != EXPECTED_ROLES:
        raise ValueError(f"Unexpected panel roles: observed={dict(role_counts)}, expected={EXPECTED_ROLES}")
    ...
    summary = {
        "contract_version": "chang2026_gene_tree_workflow_v4_canonical_restartable_sra",
        ...
        "sra_execution_contract": (
            "prefetch -> vdb-validate -> fasterq-dump local accession directory with explicit scratch"
        ),
        "trinity_execution_contract": "retain output directory and Trinity.fasta; no --full_cleanup",
        "execution_gate": (
            "The contract validates official layout, inputs, the single canonical restartable "
            "assembly path and DAG structure. Raw-read download, assembly, orthology and gene-tree "
            "inference remain explicit external steps."
        ),
    }
...
def main() -> int:
    args = parse_args()
    config, summary = build_contract(args.panel, args.hypotheses, args.snakefile, args.results_dir)
    args.config_output.parent.mkdir(parents=True, exist_ok=True)
    args.config_output.write_text(json.dumps(config, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    ...
    print("official_library_layout_counts=" + json.dumps(summary["official_library_layout_counts"], sort_keys=True))
