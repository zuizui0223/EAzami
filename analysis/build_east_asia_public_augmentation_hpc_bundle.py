...
def summarize_cross_mapping()->str:return '''#!/usr/bin/env bash
#SBATCH --job-name=EAzami-eaaug-summary
#SBATCH --cpus-per-task=1
#SBATCH --mem=4G
#SBATCH --time=01:00:00
'''+common()+'''for MODE in bwa blastx; do test -s "$AUGMENT_ROOT/$MODE/paired_inputs/paired_augmentation_summary.json"; test -d "$AUGMENT_ROOT/$MODE/evaluation"; done; "${RUN[@]}" python "$BUNDLE_DIR/helpers/summarize_east_asia_public_augmentation_sensitivities.py" --bwa-paired-summary "$AUGMENT_ROOT/bwa/paired_inputs/paired_augmentation_summary.json" --blastx-paired-summary "$AUGMENT_ROOT/blastx/paired_inputs/paired_augmentation_summary.json" --bwa-evaluation "$AUGMENT_ROOT/bwa/evaluation" --blastx-evaluation "$AUGMENT_ROOT/blastx/evaluation" --output "$AUGMENT_ROOT/cross_mapping_sensitivity_summary.json"
'''
