...
def figure1(out: Path) -> list[Path]:
    ...
    ax.text(0.02, 0.02, "36/38 sampled concepts\nin dominant Japanese radiation", transform=ax.transAxes, color=MID, fontsize=7.5)
...
def figure5(out: Path) -> list[Path]:
    ...
    ax.add_patch(plt.Rectangle((0.24, 0.42), 0.52, 0.17, facecolor="#E8F0EC", edgecolor=TEAL, lw=1.5, transform=ax.transAxes))
    ax.text(0.5, 0.505, "Partial coordinated remodelling\ninside modular, lineage-dependent histories", transform=ax.transAxes, ha="center", va="center", fontsize=8, fontweight="bold")
