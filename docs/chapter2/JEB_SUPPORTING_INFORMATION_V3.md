...
The same 1,000 UFBoot topology realizations were compared directly.
...
The predeclared U→D present-niche vector was `BIO15 up + BIO1 down`. The primary statistic combines branchwise transition probabilities from a symmetric two-state CTMC with reconstructed branch differences in present-day taxon niche centroids.
