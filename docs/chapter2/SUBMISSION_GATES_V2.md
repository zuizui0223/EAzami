...
The current paper can stop at:

> **Cirsium capitulum diversity is not well represented as one adaptive syndrome. Its decomposed phenotype components have distinct candidate functions and repeated, partly decoupled histories on the current nuclear topology ensemble. Repeated states are established as recurrence lower bounds, but their historical origin and adaptive convergence remain unresolved.**

This paper does not need to prove adaptive convergence.
...
6. **Make the convergence boundary explicit in title/abstract/discussion**
   - repeated state ≠ independent origin;
...
- Figure 1: decomposed capitulum phenotype and within/among organization;
- Figure 2: phenotype → candidate-function evidence map;
- Figure 3: Japan38 nuclear tree with separate orientation/phyllary/stickiness histories;
- Figure 4: repeated-state → origin → function → adaptive-convergence claim ladder.

The 62-target simulation belongs in supplement or a late auxiliary panel unless the target journal specifically rewards methodological inverse-problem framing.
...
- JPN36/JPN15 field manipulation outcomes;
- floral RNA-seq;
- pigment chemistry;
- absolute-time chronogram;
- proof of adaptive radiation;
- proof of modular evolvability.

These are next-stage discriminators, not missing controls for a carefully bounded current paper.
...
`standardized phenotype + nuclear population ancestry + matched plastid haplotype + cytotype/genome size`.
...
- relevant trade-off channel measured at the same time.

Only then may repeated evolution be described as adaptive convergence.

## Stop rules

- do not delay the bounded current paper merely to obtain all population/genomic/field evidence required for a stronger future convergence paper;
- do not call literature functional annotation a focal functional validation;
- do not turn the simulation auxiliary lane back into the definition of evolutionary history;
- do not re-open the source-balanced colour anti-phylogenetic hypothesis solely to obtain a preferred sign;
- do not add another generic climate model unless it changes phenotype → function, history, origin or convergence discrimination.
