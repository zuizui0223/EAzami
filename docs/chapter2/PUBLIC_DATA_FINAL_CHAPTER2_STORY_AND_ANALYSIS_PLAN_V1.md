...
Use published node-age scaffolds only where taxon/clade reconciliation is explicit. The current strongest case is the Taiwan/Nipponocirsium orientation event:

- C. morii = U;
- C. pengii, C. kawakamii, C. tatakaense = D;
- parent split = 0.79 Ma;
- Taiwan-trio crown = 0.47 Ma;
- one minimum U→D event is therefore forced onto the 0.79–0.47 Ma lineage under the published local scaffold.

This does not date all 4–6 Japan38 orientation changes. It provides one public-data event-level test.
...
Azami spatial evidence is strong for RSDS–chroma, but Chapter 2 lacks a defensible repeated population/morph-matched colour-event history and a directly equivalent historical surface-RSDS series. Priority is to recover public morph-linked colour states on dated East-Asian samples, not to force continuous Japanese non-support into a colour-transition story. If repeated dated colour events become identifiable, use independent orbital/solar or radiative proxy models as a clearly separate model layer.
...
Search public literature/supplements/raw sequence panels for additional exactly reconciled dated orientation/phyllary/stickiness/colour subclades. Goal: multiple independent event windows rather than one Taiwan event.

### P2 — Build event-by-process registry

For every usable event record trait, transition direction/state, parent/child age interval, topology support, biogeographic context, paleolocation scenarios and whether the event is forced/optional under minimum histories.

### P3 — Run state–trajectory tests across all dated events

For every event, compute the frozen shared/core variables, trait-specific mechanism variables and matched-window nulls. Aggregate only after event-level results are retained.
...
Recover taxon-level flowering months from authoritative/public floras. Where event lineage phenology can be bounded, derive monthly/anthesis-window PALEO-PGEM temperature/precipitation instead of relying only on annual BIOCLIM. This is especially valuable for orientation because rain-on-reproductive-structures is season-specific.

### P6 — Distribution-trigger analysis

Use public biogeographic reconstructions and sea-level/land-connectivity histories to classify whether trait events followed colonization, fragmentation or within-region diversification. Test “range change opens a new selective environment” rather than treating range change itself as selection.
