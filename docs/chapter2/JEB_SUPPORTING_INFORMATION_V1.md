...
![Alternative text: Maximum-likelihood phylogram of the Japan38 Comp1061 panel with branch length in substitutions per site. JPN20 is represented by two separate tips, JPN29 carries an identity-provenance warning, and JPN31 is crossed out as excluded from primary trait history.](figures/supplementary/figure_s1_tree_admission.png)
...
![Alternative text: Two-panel forest plot of patristic-distance versus trait-distance Spearman correlations. Original families at two observation thresholds are shown on the left and the fixed JPN29-excluded sensitivity on the right. All evaluable rows are not supported, and the high-depth JPN29-excluded family is explicitly marked not evaluable.](figures/supplementary/figure_s2_continuous_state_structure.png)
...
![Alternative text: Interval summaries of equal-branch topology sensitivity for eight continuous traits. Every topology gives a positive mean branch-change correlation, but the panel is labelled diagnostic only because it includes JPN29 and lacks a reconstruction-aware topology-specific null.](figures/supplementary/figure_s3_equal_branch_topology.png)
...
![Alternative text: Pairwise Japanese source-balanced corolla lightness differences plotted against patristic distances. The observed correlation is positive, the exact predeclared negative-tail test fails, and all leave-one-out correlations remain positive.](figures/supplementary/figure_s4_lightness_nonreplication.png)
...
![Alternative text: Information ladder for one Cirsium japonicum variety system. The species-tip tree permits a minimum of one change, the population-aware sensitivity permits two, four of four tests support state compression, one of four supports a genealogy statement, and none supports a replicated rate statement.](figures/supplementary/figure_s5_species_tip_compression.png)
