...
ROOT=Path(__file__).resolve().parents[1];MOD=ROOT/'analysis/validate_fixed_white_public_nuclear_recovery_audit.py'
spec=importlib.util.spec_from_file_location('fwpub',MOD);assert spec and spec.loader;m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
...
class FixedWhitePublicAuditTests(unittest.TestCase):
 def fixture(self,root:Path,*,bad_status:bool=False):
  p=root/'audit.csv';rows=[
   ...
   ['WREC01','Cirsium boninense','remote copy','034039888 p69','ndl_remote_copy_route_confirmed','NDL','x','requestable exact page','request p69','no order placed'],
  ...
  with p.open('w',newline='',encoding='utf-8') as h:w=csv.writer(h);w.writerow(FIELDS);w.writerows(rows)
  return p
 def test_current_discovery_states_do_not_promote_tip(self):
  with tempfile.TemporaryDirectory() as td:
   out=m.validate(self.fixture(Path(td)));self.assertFalse(out['usable_nuclear_tip_recovered']);self.assertFalse(out['rate_fit_tip_promotion_allowed']);self.assertTrue(out['boninense_ndl_p69_remote_copy_route_confirmed']);self.assertFalse(out['boninense_ndl_copy_order_placed']);self.assertFalse(out['wulongense_published_study_contains_molecular_analysis']);self.assertEqual(out['evidence_rows'],15);self.assertEqual(out['contract_version'],'fixed_white_public_nuclear_recovery_audit_v4')
 def test_rejects_ready_status_in_discovery_audit(self):
  with tempfile.TemporaryDirectory() as td:
   with self.assertRaisesRegex(ValueError,'unsupported result_status'):m.validate(self.fixture(Path(td),bad_status=True))
