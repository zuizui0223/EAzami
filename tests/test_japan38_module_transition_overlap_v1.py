...
class TestJapan38ModuleTransitionOverlap(unittest.TestCase):
    def build_fixture(self, root: Path):
        ...
        fields = ["paper_japan_member_id", "orientation_state", "phyllary_posture", "stickiness_state"]
        tr = [
            {"paper_japan_member_id":"JPN_01","orientation_state":"upward_or_erect","phyllary_posture":"appressed","stickiness_state":"sticky"},
            {"paper_japan_member_id":"JPN_02","orientation_state":"upward_or_erect","phyllary_posture":"ascending","stickiness_state":"sticky"},
            {"paper_japan_member_id":"JPN_03","orientation_state":"downward_or_nodding","phyllary_posture":"spreading","stickiness_state":"nonsticky_or_nearly_nonsticky"},
            {"paper_japan_member_id":"JPN_04","orientation_state":"downward_or_nodding","phyllary_posture":"spreading_or_recurved","stickiness_state":"nonsticky_or_nearly_nonsticky"},
            {"paper_japan_member_id":"JPN_20","orientation_state":"downward_or_nodding","phyllary_posture":"ascending_or_recurved","stickiness_state":"sticky"},
        ]
        with trait.open("w", newline="", encoding="utf-8") as h:
            w=csv.DictWriter(h, fieldnames=fields); w.writeheader(); w.writerows(tr)
    ...
    def test_fit_and_overlap_keep_missing_ambiguous(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            fits={t:jt.fit_trait(tree,states[t],jt.STATE_UNIVERSE[t]) for t in jt.STATE_UNIVERSE}
            ...
            self.assertTrue(any(e["informative"] for e in fits["orientation"]["edges"]))
            ...
            for row in comp.values():
                ...
                p=row["branch_length_stratified_one_sided_p_for_positive_overlap"]
                if p is not None:
                    self.assertGreaterEqual(p,0); self.assertLessEqual(p,1)

    def test_nonmonophyletic_jpn20_is_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            root=Path(d)
            tree_path, concept_path, trait_path=self.build_fixture(root)
            txt=tree_path.read_text()
            # Break the exclusive JPN20 cherry without changing the tip set.
            txt=txt.replace("(S20a:0.01,S20b:0.01):0.02", "(S20a:0.01,S01:0.011):0.02")
