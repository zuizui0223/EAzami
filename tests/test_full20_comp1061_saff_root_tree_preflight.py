...
class Full20Comp1061SaffRootTreePreflightTest(unittest.TestCase):
    def test_frozen_pre_topology_locus_contract(self):
        ...
        self.assertEqual(x["reference_contract"]["tree_reference_tips"], ["OUTGROUP_saff"])
        self.assertEqual(x["reference_contract"]["audited_but_excluded_distant_reference_tips"], ["OUTGROUP_lett", "OUTGROUP_sunf"])
        ...
        self.assertEqual(x["alignment_policy"]["expected_tree_tips_when_complete"], 21)
        self.assertFalse(x["alignment_policy"]["posthoc_signal_filtering_allowed"])
        self.assertEqual(x["tree_policy"]["tree_reference_tips"], ["OUTGROUP_saff"])
