...
class MaximumPublicCombinedTreeInputsTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.baseline = cls.handoff / "ea01_handoff/baseline_bundle"
        cls.ea01 = cls.handoff / "ea01_handoff/augmentation_bundle/candidate_packs/EA01"
        ...
        common = [x for x in e1 if x in cn]
        if len(common) < 100:
            raise AssertionError(f"durable baseline-EA01-CNIPG candidate intersection unexpectedly below 100: {len(common)}")
        ...
        cls.gate.write_text(json.dumps({
            ...
            "independent_candidate_gate_results": {"EA01": True, "CNIPG": True},
            "independent_manual_review_required": {"EA01": False, "CNIPG": False},
            "excluded_duplicate_controls": {"EA02": "duplicate_readset_pseudoreplicate_excluded_pending_explicit_provenance"},
            "both_independent_gates_passed": True,
    ...
    def test_all_four_scenarios_use_one_common_locus_set(self):
        summary, out = self.prepare()
        self.assertTrue(summary["independent_gate_prerequisite_passed"])
        self.assertEqual(summary["baseline_ea01_cnipg_common_paired_loci"], 100)
        self.assertEqual(summary["scenario_count"], 4)
        self.assertTrue(summary["all_four_scenarios_use_identical_locus_set"])
        self.assertFalse(summary["ea02_enters_combined_tree"])
        ...
        self.assertEqual(summary["scenario_focal_tip_counts"]["ea01_cnipg_296"], 296)
        self.assertEqual(len((out / "ea01_cnipg_296/eligible_loci.txt").read_text().splitlines()), 100)
        self.assertEqual(len((out / "ea01_cnipg_296/primary_runs.csv").read_text().splitlines()) - 1, 296)
        self.assertEqual(len(list((out / "ea01_cnipg_296/loci_unaligned").glob("*.fasta"))), 100)

        import csv
        rows = list(csv.DictReader((out / "ea01_cnipg_296/astral_species_map.csv").open()))
        ...
        self.assertNotIn("PUBEA002", "|".join(row["tip_ids"] for row in rows))
    ...
    def test_independent_gate_failure_blocks_combined_inputs(self):
        ...
        data["independent_candidate_gate_results"]["CNIPG"] = False
        data["both_independent_gates_passed"] = False
        data["combined_common_paired_locus_tree_required"] = False
        bad.write_text(json.dumps(data))
        with self.assertRaisesRegex(ValueError, "EA01 and CNIPG must both pass"):
