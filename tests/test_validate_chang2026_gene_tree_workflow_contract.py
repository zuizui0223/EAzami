...
class ChangGeneTreeWorkflowContractTests(unittest.TestCase):
    def setUp(self):
        ...
        self.snakefile = REPO_ROOT / "workflow" / "chang2026_gene_trees" / "Snakefile"
    ...
    def _panel_rows(self):
        specs = [
            ...
            ("C. japonicum var. australe", "coloured_flanking_introgression_control", 3),
            ("C. japonicum var. fukienense", "coloured_flanking_introgression_control", 4),
            ("C. japonicum", "coloured_root_context", 2),
            ("C. lineare", "outgroup", 2),
        ]
        focal = [
            ("FC", "ccy3559", "BP"), ("TJ", "ccy3807", "BP"), ("NH", "ccy3835", "BP"),
            ("WY", "ccy3560", "W"), ("FB", "ccy3629", "W"), ("LT", "ccy3839", "W"),
        ]
        rows = []
        counter = 0
        for taxon, role, count in specs:
            for within in range(count):
                ...
                if role == "focal_colour_morph":
                    ...
                    code = f"S{counter:02d}"; voucher = f"ccy{8000 + counter}"; morph = ""
                run = f"SRR{counter:08d}"
                rows.append({
                    "sample_id": f"{code}_{voucher}", "taxon": taxon, "code": code, "voucher": voucher,
                    "morph": morph,
                    "flower_colour_state": "white" if morph == "W" or "albescens" in taxon else "bluish-purple" if morph == "BP" else "coloured",
                    "panel_role": role, "matched_run": run, "matched_experiment": f"SRX{counter:08d}",
                    "matched_biosample": f"SAMN{counter:08d}", "library_layout": "PAIRED",
                    "matched_spots": str(counter * 1000),
                    "read_count_relation": "exact_paired_end_raw_reads_equals_2x_spots" if counter <= 9 else "not_matching_reported_raw_reads",
                    "run_match_status": "verified_unique_voucher_token", "run_match_confidence": "verified",
                    "public_transcriptome_status": "not_recovered_by_current_ncbi_query",
                    "preferred_sequence_source": run, "tsa_accessions": "", "assembly_accessions": "",
                    "de_novo_required": "true", "analysis_panel": "sinocirsium17_plus_lineare2",
                })
        return rows

    def _hypothesis_rows(self):
        nearest = REPO_ROOT / "analysis" / "chang2026_takaoense_nearest_no_regain_topologies.csv"
        summary = REPO_ROOT / "analysis" / "chang2026_takaoense_topology_robustness_summary.json"
    ...
    def test_actual_snakefile_has_complete_ordered_dag(self):
        rules, runners, envs = mod.validate_workflow_files(self.snakefile)
        self.assertEqual(rules, list(mod.EXPECTED_RULES))
        self.assertEqual(set(runners), set(mod.EXPECTED_RUNNERS))
        self.assertEqual(set(envs), set(mod.EXPECTED_ENVS))
        self.assertEqual(
            set(runners),
            {
    ...
    def test_contract_accepts_nineteen_samples_and_eight_hypotheses(self):
        config, summary = mod.build_contract(self.panel, self.hypotheses, self.snakefile, self.root / "results")
        ...
        self.assertEqual(summary["contract_version"], "chang2026_gene_tree_workflow_v4_canonical_restartable_sra")
        self.assertIn("prefetch", summary["sra_execution_contract"])
        self.assertIn("no --full_cleanup", summary["trinity_execution_contract"])
    ...
    def test_read_count_mismatch_does_not_invalidate_official_layout(self):
        config, summary = mod.build_contract(self.panel, self.hypotheses, self.snakefile, self.root / "results")
        self.assertEqual(sum(r["read_count_relation"] == "not_matching_reported_raw_reads" for r in self.panel_rows), 10)
        self.assertEqual(summary["official_library_layout_counts"], {"PAIRED": 19})
        self.assertTrue(config["panel_csv"].endswith("panel.csv"))

    def test_official_single_layout_fails_current_heavy_contract(self):
        rows = [dict(r) for r in self.panel_rows]
        ...
        with self.assertRaisesRegex(ValueError, "not PAIRED"):
            mod.build_contract(wrong, self.hypotheses, self.snakefile, self.root / "results")

    def test_duplicate_hypothesis_topology_fails(self):
        rows = [dict(r) for r in self.hypothesis_rows]
    ...
    def test_rule_parser_rejects_missing_stage(self):
        observed = mod.extract_rule_names(
            self.snakefile.read_text(encoding="utf-8").replace("rule orthofinder:", "# removed orthofinder:")
        )
