...
import run_chang2026_restartable_transcriptome_assembly as assembly_runner  # noqa: E402
...
class ChangGeneTreeWorkflowIntegrationTests(unittest.TestCase):
    ...
    def _panel_rows(self) -> list[dict[str, str]]:
        ...
        focal_codes = [
            ("FC", "3559", "BP"), ("TJ", "3807", "BP"), ("NH", "3835", "BP"),
            ("WY", "3560", "W"), ("FB", "3629", "W"), ("LT", "3839", "W"),
        ]
    ...
    def _hypothesis_rows(self) -> list[dict[str, object]]:
        nearest_path = REPO_ROOT / "analysis" / "chang2026_takaoense_nearest_no_regain_topologies.csv"
        summary_path = REPO_ROOT / "analysis" / "chang2026_takaoense_topology_robustness_summary.json"
    ...
    def test_end_to_end_offline_interfaces(self) -> None:
        # 1. Validate the 19-sample official-layout SRA panel and dry-run the
        # canonical restartable command plans without invoking external tools.
        validated = assembly_runner.validate_panel(self.panel_path, expected_samples=19)
        ...
        assembly_results = [
            assembly_runner.execute_one(
                plan,
                dry_run=True,
                force=False,
                delete_raw_after_success=False,
                delete_sra_after_success=False,
            )
        ...
        (orthogroups / "Orthogroups_SingleCopyOrthologues.txt").write_text("OG0001\n", encoding="utf-8")
        ...
        manifest, og_summary = prepare_ogs.prepare(self.root / "orthofinder", self.panel_path, og_out)
        self.assertEqual(og_summary["complete_single_copy_count"], 1)
        manifest_path = og_out / "single_copy_orthogroup_manifest.csv"
        prepare_ogs.write_csv(manifest_path, manifest, prepare_ogs.MANIFEST_FIELDS)
        ...
        published_samples = str(self.hypothesis_rows[0]["topology_newick"])
        ...
        (tree_dir / "OGpub.treefile").write_text(published_samples, encoding="utf-8")
        (tree_dir / "OGloss.treefile").write_text(loss_samples, encoding="utf-8")
