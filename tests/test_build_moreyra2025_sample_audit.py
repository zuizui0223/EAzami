...
MODULE_PATH = (
    ...
    / "analysis"
    / "build_moreyra2025_sample_audit.py"
)
SPEC = importlib.util.spec_from_file_location(
    "build_moreyra2025_sample_audit", MODULE_PATH
)
...
class MoreyraAuditTests(unittest.TestCase):
    def test_region_scope_separates_far_east_from_caucasus(self) -> None:
        self.assertEqual(
            mod.classify_region_text("Russia: Primorskiy territory"),
            ("Russian_Far_East", "northeast_asia_bridge"),
        )
        self.assertEqual(
            mod.classify_region_text("Russia Caucasus: North Ossetia"),
            ("Russia_other", "outside_scope"),
        )
        self.assertEqual(
            mod.classify_region_text("Japan: Honshu"),
            ("Japan", "core_east_asia"),
        )

    def test_geographic_conflict_is_retained(self) -> None:
        result = mod.combine_regions("Ukraine", "Japan. Honshu")
        self.assertEqual(result[2], "Japan|Outside_target_region")
        self.assertEqual(result[3], "source_conflict_target_vs_outside")
        self.assertEqual(result[4], "conflicting_resolved_regions")
    ...
    def test_join_keeps_supplement_row_without_run(self) -> None:
        ...
        self.assertEqual(rows[0]["geographic_evidence_relation"], "supplement_only")
